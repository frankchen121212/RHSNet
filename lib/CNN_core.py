...
class MotifMirrorGradientBleeding(keras.callbacks.Callback):
    ...
    def ass_bias(self):
        if self.ass_b is None:
            self.ass_b = self.model.weights[self.weight_layer + 1][self.split_ix:].assign(
                self.model.weights[self.weight_layer + 1][:self.split_ix][::-1])
        return self.ass_b
    ...
    def bleed_rc_gradient(self):
        if self.bleed is None:
            self.bleed = self.model.weights[self.weight_layer][:, :, :self.split_ix].assign(
                tf.add(tf.scalar_mul(tf.constant(1.0) - tf.minimum(self.x, self.m), self.cached_constrained_rc())
                       , tf.scalar_mul(tf.minimum(self.x, self.m), self.cached_unconstrained())))
        self.i += self.gradient_increment
        return self.bleed

    def on_batch_end(self, batch, logs={}):
        assert self.model.weights[self.weight_layer].get_shape().as_list()[
                   2] % 2 == 0, "Input must have an even number of filters"
